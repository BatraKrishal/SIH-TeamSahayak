...
def ask():
    ...
    try:
        ...
        table_html = table_html.replace(
            "<table",
            '<table style="width:100%; border-collapse: collapse; color: white; background-color: #0F284A;"'
        ).replace(
            "<th",
            '<th style="border: 1px solid #4b5563; padding: 8px; background-color: #0F284A;"'
        ).replace(
            "<td",
